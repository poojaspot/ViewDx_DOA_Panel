...
def process_doa_image(total_tests,image, sampleid, date):
    ...
    try:
        ...
        print("casstte_image xy",cassette_image.shape)
        bottom_height = 650
        test_window = cassette_image[x-bottom_height:x-20,70:-60]
#         test_window = cassette_image[x//2+70:-15,70:-60]
        ...
        for contour in test_contours:
            if cv2.contourArea(contour) > 3000:
                print("cnt_Area",cv2.contourArea(contour))
        ...
        for contour in cleaned_contours:
            ...
            if len(approx)>=4:
                perimeter = cv2.arcLength(contour, True)
                if perimeter > 300:
                    print("cnt_Area",cv2.contourArea(contour), cv2.arcLength(contour, True))
                    tx, ty, tw, th = cv2.boundingRect(contour)
                    
                    aspect_ratio = tw / float(th) if th != 0 else 0
                    print("326 PR: ",perimeter,"AR: ",aspect_ratio,"th: ",th,"tw: ",tw)
#                     print("perimeter",perimeter,"aspect_ratio",aspect_ratio,"th",th,"tw",tw)
                    if 0.1 <= aspect_ratio <= 0.98:
                        print("330ARFILTER PR: ",perimeter,"AR: ",aspect_ratio,"th: ",th,"tw: ",tw)
        ...
        if len(test_rects) != total_tests: #add logic
...
def filter_result_array(result_array, window=15):
...
def read_test(data_array, overwrite):
    ...
    try:
        print(analyte)
        doa_panels_lower = {panel.lower(): panel for panel in deviceinfo.doa_panels}
        analyte_lower = analyte.lower()
        print(doa_panels_lower,analyte_lower)
        if analyte_lower in doa_panels_lower:
            ...
            total_tests = deviceinfo.doa_panels[panel_name]
