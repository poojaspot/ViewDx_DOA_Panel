...
#-------------------------------------------Settings screens---------------------------------------------------
    ...
    def send_data():
        ...
        results.usesummary("Result data is shared with LMS system.")
    ...
    def DeviceData():
        ...
        def start_update():
            splash.destroy()
            subprocess.Popen([sys.executable,"/home/pi/viewdx/updater.py"])
            sys.exit(0)
...
#         settings.mainloop()
    ...
    def WifiInfo():
        print('here')
        returncode = email_proto.ping()
        print(returncode)
        if returncode ==0:
           widgets.error("No action needed. Device is connected to wifi.")
        else:
            ...
            def find_ssid():
                try:
                    ...
                    ssid_list = []
                    widgets.error(f"No networks detected. Scan again.")
...
#         barcode_reader.start_scanner_thread_qr()
...
#---------------------------------------------------------------------------------------------------------------------- 
    ...
    def Homepage():
        ...
        Label(homepage, text = "Sample ID",font = screen_config.menufont, background = "white").place(relx=0.15,rely=0.25,anchor=CENTER)
...
#         Button(homepage, bd=0, text = "Scan Tube", font = screen_config.menufont, justify="left", width =10, command = barcode_reader.start_scanner_thread(sampleidE)).place(relx=0.25,rely=0.40)
        analytelist = ['Add New']
        analytedb = TinyDB(deviceinfo.path+'analytes.json')
    ...
    def PatientData(data_array):
        ...
        searchlist = db.search(Sample.sampleid==str(data_array[0]))
        for s in searchlist:
            if s.get('name','').strip() and not nameE.get().strip():
                nameE.insert(0,s['name'])
                referE.insert(0,s['refer'])
                ageE.insert(0,s['age'])
                gendrop.set(s['gender'])
    ...
    def ResultView():
        ...
        try:
            n_pages = math.ceil(len(result_list)/per_page)
        ...
        def update_list(pagenumber):
            ...
            for l in list_page:
                ...
                if isinstance(result_val,list):
                    result_val = ",".join(result_val)
                readings.append((l['sampleid'],l['name'],l['analyte'], result_val, (str(l['date'])+'.')))
...
#                 widgets.askquestion(q, delr, str(sampleid))
        ...
        Button(button_frame, text = "Prev", font = screen_config.labelfont, background="white", width=6, bd=0, command = lambda: prev_btn()).pack(side = LEFT)
        Button(button_frame, text = "Next", font = screen_config.labelfont, background="white", width=6, bd=0, command = lambda: next_btn()).pack(side = LEFT)
    ...
    def SearchView():
        ...
        def Search():
            global search_list, pagenumber,n_pages, per_page, searchE
            s = str(searchE.get())
            exp = s
            if not s:
               widgets.error("Please enter a search item")
            arr = TinyDB(deviceinfo.path+'results/results.json')
            Sample=Query()
            Querymod = ((Sample.sampleid.matches(exp, flags=re.IGNORECASE))|
                       (Sample.name.matches(exp,flags=re.IGNORECASE))|
                       (Sample.analyte.matches(exp,flags=re.IGNORECASE))|
                       (Sample.date.matches(exp,flags=re.IGNORECASE))|
                       (Sample.result.matches(exp,flags=re.IGNORECASE)))
            results.usesummary("Results searched for: "+exp)

            search_list = arr.search(Querymod)[::-1]

            if len(search_list)==0:
               widgets.error("No reports found")
               clear_all()
            pagenumber = 0
            n_pages = math.ceil(len(search_list)/per_page)
            print(n_pages)
            if len(search_list)==0:
                widgets.error("No results found")
            else: 
                string = 'Number of results fetched: ' +str(len(search_list))
                widgets.error(string)
            update_list()
...
#                 x = deviceinfo.path+'results/'+str(sampleid)+'_'+str(analyte)+'_'+date+'.pdf'
        ...
        Button(search_frame, text = "Search", font = screen_config.smallfont, background="white", width=6, command = lambda: Search()).pack(side = LEFT)
        Button(search_frame, text = "Prev", font = screen_config.smallfont, background="white", width=5, command = lambda: prev_btn()).pack(side = LEFT)
        Button(search_frame, text = "Next", font = screen_config.smallfont, background="white", width=5, command = lambda: next_btn()).pack(side = LEFT)
